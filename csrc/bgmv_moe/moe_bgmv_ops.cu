...
void dispatch_bgmv_moe_shrink(torch::Tensor y, torch::Tensor x, torch::Tensor w_ptr,
                              torch::Tensor sorted_token_ids, torch::Tensor expert_ids,
                              torch::Tensor lora_indices, int64_t lora_stride) {
  CHECK_INPUT(y);
  CHECK_INPUT(x);
  CHECK_INPUT(w_ptr);
  ...
  switch (x.scalar_type()) {
    case at::ScalarType::Half:
      ok = launch_moe_shrink_sliced_kernel(
          static_cast<nv_half*>(y.data_ptr()), static_cast<nv_half*>(x.data_ptr()),
          reinterpret_cast<nv_half**>(w_ptr.data_ptr<int64_t>()),
          sorted_token_ids.data_ptr<int64_t>(), expert_ids.data_ptr<int64_t>(),
          lora_indices.data_ptr<int64_t>(), feat_in, feat_out, num_pairs, num_slices, num_experts,
          num_tokens, lora_stride);
      break;
    case at::ScalarType::BFloat16:
      ok = launch_moe_shrink_sliced_kernel(
          static_cast<nv_bfloat16*>(y.data_ptr()), static_cast<nv_bfloat16*>(x.data_ptr()),
  ...
  switch (x.scalar_type()) {
    case at::ScalarType::Half:
      ok = launch_moe_expand_sliced_kernel(
          static_cast<float*>(y.data_ptr()), static_cast<nv_half*>(x.data_ptr()),
          reinterpret_cast<nv_half**>(w_ptr.data_ptr<int64_t>()),
          sorted_token_ids.data_ptr<int64_t>(), expert_ids.data_ptr<int64_t>(),
          lora_indices.data_ptr<int64_t>(), topk_weights.data_ptr<float>(),
          slice_start_loc.data_ptr<int64_t>(), feat_in, first_feat_out, num_pairs, num_slices,
          num_experts, total_feat_out, num_tokens, lora_stride);
      break;
    case at::ScalarType::BFloat16:
      ok = launch_moe_expand_sliced_kernel(
          static_cast<float*>(y.data_ptr()), static_cast<nv_bfloat16*>(x.data_ptr()),
          reinterpret_cast<nv_bfloat16**>(w_ptr.data_ptr<int64_t>()),
          sorted_token_ids.data_ptr<int64_t>(), expert_ids.data_ptr<int64_t>(),
          lora_indices.data_ptr<int64_t>(), topk_weights.data_ptr<float>(),
          slice_start_loc.data_ptr<int64_t>(), feat_in, first_feat_out, num_pairs, num_slices,
