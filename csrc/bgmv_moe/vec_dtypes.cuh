...
template <typename src_float_t, typename tgt_float_t, size_t vec_size>
BGMV_MOE_INLINE void cast_load_impl(const src_float_t *src_ptr,
                                     vec_t<tgt_float_t, vec_size> &dst) {
...
struct vec_t<half, 1> {
  ...
  template <typename T> BGMV_MOE_INLINE void cast_from(const vec_t<T, 1> &src) { cast_from_impl(src, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
...
struct vec_t<half, 2> {
  ...
  template <typename T> BGMV_MOE_INLINE void cast_from(const vec_t<T, 2> &src) { cast_from_impl(src, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
  BGMV_MOE_INLINE static void memcpy(half *dst, const half *src) { *((half2 *)dst) = *((half2 *)src); }
};
...
struct vec_t<half, 4> {
  ...
  BGMV_MOE_INLINE half &operator[](size_t i) { return ((half *)(&data))[i]; }
  BGMV_MOE_INLINE const half &operator[](size_t i) const { return ((const half *)(&data))[i]; }
  BGMV_MOE_INLINE void fill(half val) { *(half2 *)(&data.x) = make_half2(val, val); *(half2 *)(&data.y) = make_half2(val, val); }
  BGMV_MOE_INLINE void load(const half *ptr) { data = *((uint2 *)ptr); }
  BGMV_MOE_INLINE void store(half *ptr) const { *((uint2 *)ptr) = data; }
  template <typename T> BGMV_MOE_INLINE void cast_from(const vec_t<T, 4> &src) { cast_from_impl(src, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
  BGMV_MOE_INLINE static void memcpy(half *dst, const half *src) { *((uint2 *)dst) = *((uint2 *)src); }
};
...
#pragma unroll
    for (size_t i = 0; i < vec_size / 8; ++i) { data[i] = ((uint4 *)ptr)[i]; }
  }
  BGMV_MOE_INLINE void store(half *ptr) const {
#pragma unroll
    for (size_t i = 0; i < vec_size / 8; ++i) { ((uint4 *)ptr)[i] = data[i]; }
  }
  template <typename T> BGMV_MOE_INLINE void cast_from(const vec_t<T, vec_size> &src) { cast_from_impl(src, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
  BGMV_MOE_INLINE static void memcpy(half *dst, const half *src) {
#pragma unroll
    for (size_t i = 0; i < vec_size / 8; ++i) { ((uint4 *)dst)[i] = ((uint4 *)src)[i]; }
  }
};
...
struct vec_t<nv_bfloat16, 1> {
  ...
  BGMV_MOE_INLINE const nv_bfloat16 &operator[](size_t i) const { return ((const nv_bfloat16 *)(&data))[i]; }
...
struct vec_t<nv_bfloat16, 2> {
  ...
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
  BGMV_MOE_INLINE static void memcpy(nv_bfloat16 *dst, const nv_bfloat16 *src) { *((nv_bfloat162 *)dst) = *((nv_bfloat162 *)src); }
};
...
struct vec_t<nv_bfloat16, 4> {
  ...
  BGMV_MOE_INLINE nv_bfloat16 &operator[](size_t i) { return ((nv_bfloat16 *)(&data))[i]; }
  BGMV_MOE_INLINE const nv_bfloat16 &operator[](size_t i) const { return ((const nv_bfloat16 *)(&data))[i]; }
  BGMV_MOE_INLINE void fill(nv_bfloat16 val) { *(nv_bfloat162 *)(&data.x) = make_bfloat162(val, val); *(nv_bfloat162 *)(&data.y) = make_bfloat162(val, val); }
  ...
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
  BGMV_MOE_INLINE static void memcpy(nv_bfloat16 *dst, const nv_bfloat16 *src) { *((uint2 *)dst) = *((uint2 *)src); }
};
...
struct vec_t<float, 2> {
  ...
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
  BGMV_MOE_INLINE static void memcpy(float *dst, const float *src) { *((float2 *)dst) = *((float2 *)src); }
};
...
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) { data[i] = make_float4(val, val, val, val); }
  }
  BGMV_MOE_INLINE void load(const float *ptr) {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) { data[i] = ((float4 *)ptr)[i]; }
  }
  BGMV_MOE_INLINE void store(float *ptr) const {
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) { ((float4 *)ptr)[i] = data[i]; }
  }
  template <typename T> BGMV_MOE_INLINE void cast_from(const vec_t<T, vec_size> &src) { cast_from_impl(src, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_load(const T *ptr) { cast_load_impl(ptr, *this); }
  template <typename T> BGMV_MOE_INLINE void cast_store(T *ptr) const { cast_store_impl(*this, ptr); }
...
#pragma unroll
    for (size_t i = 0; i < vec_size / 4; ++i) { ((float4 *)dst)[i] = ((float4 *)src)[i]; }
  }
};
...
template <size_t vec_size>
BGMV_MOE_INLINE void cast_from_impl(const vec_t<half, vec_size> &src,
